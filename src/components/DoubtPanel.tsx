...
import type { DoubtChoice, GameAction, Player } from '@/lib/types';
  ...
  return (
    <div className="doubt-panel">
      ...
      <div className="opponent-row">
        {opponents.map((p) => {
          // 투표 권한 분리: 실제 멀티플레이라면 각 클라이언트는 자기 좌석(MY_PLAYER_ID)의
          ...
          return (
            <div key={p.id} className="card opponent-choice">
              ...
              {acted ? (
                ...
                <div className="btns">
                  <button className="pill-btn swamp small" onClick={() => GameService.vote(dispatch, p.id, 'doubt')}>👁️ 의심</button>
                  <button className="pill-btn ghost small" onClick={() => GameService.vote(dispatch, p.id, 'pass')}>➡️ 진행</button>
                </div>
              ) : (
